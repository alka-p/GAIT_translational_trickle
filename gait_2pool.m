...
function gait_2pool_v3
clear
clc

global ifnogam ifn0 ydata1 ydata2 final_mrna final_dpdt ratio
...
ifn0=ifnogam(i);
        ...
        u(1)=y1(1); % gait element containing mrna (assume the input rate due to transciption surge same as pre-mrna)
        u(2)=Gy1(1); % gait bound mrna pool containing gait element
        u(3)=My1(1);
        u(4)=y2(1);
        ...
        %u(8,1)+u(8,2)+u(8,3)
        %new(8)
        total_mrna=u(tf,1)+u(tf,2)+u(tf,3) + (u(tf,1)+u(tf,2)+u(tf,3))/ratio;
        proteinsyn=new(tf);
        ...
        mrna(i)=u(tf,1)+u(tf,2)+u(tf,3) + (u(tf,1)+u(tf,2)+u(tf,3))/ratio;
        dpdt(i)=new(tf);
...
function ut=extra(t,u)
            ...
            %ratio=10; % ratio of gait element containg mrna pool to gait devoid mrna pool
            
            Mw=(u(1)+u(2)+u(3))/ratio;
            ...
            if ifn0 > cutoff
            ut(4,1)=scale2*k3*(u(1)+u(3)+Mw);
            else
            ut(4,1)=k3*(u(1)+u(3)+Mw);
            end
