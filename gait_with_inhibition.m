...
% m-rna in the active form is wither y1 or bound to M (My1)
% Inactive m-rna (Gy1)
% IFN -gamma (ifn)
...
%%%%%%%%%%%INCLUDING THE sequestration of GAIT to another complex with an
%%%%%%%%%%%inhibitor (I) leading to relaxation of gait silencing
%%%%%%%%%%% 091611

function gait_with_inhib
clear
clc
...
ifn0=ifnogam(i);
        ...
        u(1)=y1(1); %mrna
        u(2)=Gy1(1); %gait bound mrna
        u(3)=My1(1); %mini bound mrrna
        u(4)=GI(1);   % inhibitor bound gait (sequestered gait)
        u(5)=y2(1);

        u0=[u(1) u(2) u(3) u(4) u(5)];
...
% figure(1)
% plot(t,u(:,1),t,u(:,2),t,u(:,3),t,u(:,1)+u(:,3)+u(:,2));
% xlabel('time after ifn bolus (hours)')
% ylabel('concentration (AU)')
% legend('mrna-unbound','Gy1-Gait bound','My1, mini-eprs bound','total mrna','t-rna bound mini-eprs')
%   
% hold on;
%         
% figure(2)
...
function ut=extra(t,u)
            ...
            kd1=10.6; % dissociation constant for GAIT complex protein in (conc) spec in (nM)
            %k2= 0.4;%forward rate constant for y1 + M <--> My1 IN (conc.time)-1
            k2=0.75; % assuming it same as that of full length EPRS
            kd2=0.2; % dissociation constant for mini-EPRS  protein in (conc) spec in (nM)
            
            %k3=0.75;%(nMh)-1 forward rate constant for GEIF(M) + trna (T)<--->MT
            %kd3=100; %(nM) dissociation constant for GEIF(M) + trna (T)<--->MT
            
            k3=0.75;%(nMh)-1 forward rate constant for GAIT(G) + INHIBITOR (I)<--->GI
            
            
            %%%%%%%%%%VARY kd3 from 0.1 to 1000 to increase silencing due
            %%%%%%%%%%to gait%%%%%%%%%%
            
            
            kd3=10; %(nM) dissociation constant for GAIT(G) + INHIBITOR (I)<--->GI
            
            kp=2; % rate of translation of mrna to protein, (time around 30 minutes)so 2 per hour 
            ...
            if t<=32
                ...
                G=y1_ss*heaviside(t-14)-u(2)-u(4);% y1ss is the maximum value of mrna at ss after ifn stimulation
                % total GAIT protein (bound (u(2) and u(4)) + unbound remains constant
            ...
            initial_m=0;
            initial_i=800;%%%%% same as that of gait amt
           %initial_t=0;
            
            %initial_t=initial_t*heaviside(t-1);
            ...
            if t<=32
            ut(1,1)=k*mrna_max*exp(-t*k)+ k1*(-G*u(1) + kd1*u(2)) + k2*(-M*u(1) + kd2*u(3)); %active mrna
            ut(2,1)=k1*(u(1)*G - kd1*u(2)); %inactive m-rna
            else
            ut(1,1)=k*mrna_max*exp(-t*k)+ k2*(-M*u(1) + kd2*u(3)); %(y1)
            ut(2,1)=0; %(Gy1) inactive m-rna
            end
            %ut(2,1)=k1*(u(1)*G - kd1*u(2)); % inactive m-rna
            ut(3,1)=k2*(M*u(1) - kd2*u(3)); % (My1) active bound mrna
            ut(4,1)=k3*(G*I - kd3*u(4)); % (GI) inhibitor bound GAIT
            
            if ifn0 > cutoff
            ut(5,1)=scale2*kp*(u(1)+u(3));
            else
            ut(5,1)=kp*(u(1)+u(3));
            end
